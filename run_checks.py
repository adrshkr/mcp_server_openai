...
def main():
    ...
    results.append(run_command(
        ["python", "-m", "black", "--diff", "--color", "src/mcp_server_openai", "tests"],
        "Black formatting check"
    ))
    
    # 2. Black auto-format
    results.append(run_command(
        ["python", "-m", "black", "src/mcp_server_openai", "tests"],
        "Black auto-format"
    ))
    
    # 3. Ruff linting
    results.append(run_command(
        ["python", "-m", "ruff", "check", "--fix", "src/mcp_server_openai", "tests"],
        "Ruff linting"
    ))
    ...
    results.append(run_command(
        ["python", "-m", "mypy", "--config-file", "config/mypy.ini"] + core_files,
        "MyPy type checking"
    ))
    
    # 5. Fast tests
    results.append(run_command(
        ["python", "-m", "pytest", "-q", "--maxfail=1", "--durations=10", 
         "-m", "not slow and not integration and not e2e and not network", "tests"],
        "Fast tests"
    ))
