...
from mcp_server_openai.tools.enhanced_content_creator import ContentRequest, MCPClient, create_enhanced_presentation
...
async def demo_mcp_client():
    ...
    plan_result = await client.call_server(
        "sequential_thinking",
        "plan_presentation",
        {
    ...
    search_result = await client.call_server(
        "brave_search", "search_content", {"query": "AI strategy implementation best practices"}
    )
    ...
    memory_result = await client.call_server(
        "memory", "generate_content", {"slide_info": {"title": "AI Strategy Overview"}}
    )
