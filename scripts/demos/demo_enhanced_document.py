...
from mcp_server_openai.tools.enhanced_document_generator import (
    generate_document, DocumentRequest, SUPPORTED_FORMATS, DOCUMENT_TEMPLATES
)
...
This system provides enterprise-grade document generation with professional output quality."""
    ...
    for template in templates_to_test:
        try:
            ...
            if result.status == "success":
                print(f"  ✅ Successfully generated HTML document")
    ...
    try:
        ...
        result = await generate_document(
            ...
            metadata={
                "description": "Demo document with custom CSS styling",
                "author": "Enhanced Document Generator"
            }
        )
        
        if result.status == "success":
            print(f"  ✅ Successfully generated HTML with custom CSS")
            print(f"  📁 File: {result.file_path}")
            print(f"  📊 Size: {result.file_size} bytes")
            print(f"  ⏱️  Time: {result.processing_time:.2f}s")
        else:
    ...
    for lang in languages_to_test:
        try:
            ...
            result = await generate_document(
                ...
                language=lang,
                metadata={
                    "description": f"Demo document in {lang} language",
                    "author": "Enhanced Document Generator"
                }
            )
...
async def demo_advanced_features():
    ...
    try:
        ...
        if result.status == "success":
            print(f"  ✅ Successfully generated large HTML document")
            print(f"  📁 File: {result.file_path}")
            print(f"  📊 Size: {result.file_size} bytes")
            print(f"  ⏱️  Time: {result.processing_time:.2f}s")
        else:
