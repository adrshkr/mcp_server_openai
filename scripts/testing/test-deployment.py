...
class DeploymentTester:
    ...
    def __init__(self, base_url: str = "http://localhost:8080"):
        ...
        self.results = {"tests_run": 0, "tests_passed": 0, "tests_failed": 0, "failures": []}
            ...
            for endpoint, description in endpoints:
                try:
                    ...
                    if response.status_code == 200:
                        self.log_result(
                            f"Endpoint {endpoint}", True, f"({response.status_code}, {len(response.content)} bytes)"
                        )
                    else:
    ...
    async def test_health_check_details(self):
        ...
        health_endpoints = ["/health/live", "/health/ready", "/health/startup", "/status"]
                ...
                try:
                    ...
                    if response.status_code == 200:
                        ...
                        if not missing_fields:
                            self.log_result(f"Health structure {endpoint}", True, f"Status: {data.get('status')}")
                        else:
                            self.log_result(f"Health structure {endpoint}", False, f"Missing fields: {missing_fields}")
                    else:
                        self.log_result(f"Health structure {endpoint}", False, f"HTTP {response.status_code}")
        ...
        async with httpx.AsyncClient(timeout=30.0) as client:
            ...
            try:
                ...
                if response.status_code == 200 and response_time_ms < 1000:
                    self.log_result("Response time", True, f"{response_time_ms:.2f}ms")
                else:
                    self.log_result("Response time", False, f"{response_time_ms:.2f}ms (target: <1000ms)")
        ...
        try:
            async with httpx.AsyncClient(timeout=30.0) as client:
                ...
                if successful_requests == total_requests:
                    self.log_result("Concurrent requests", True, f"{successful_requests}/{total_requests} succeeded")
                else:
                    self.log_result(
                        "Concurrent requests", False, f"Only {successful_requests}/{total_requests} succeeded"
                    )
        ...
        async with httpx.AsyncClient(timeout=10.0) as client:
            try:
                ...
                if response.status_code == 404:
                    ...
                    self.log_result("404 handling", False, f"Expected 404, got {response.status_code}")
    ...
    async def run_all_tests(self):
        ...
        success_rate = (
            self.results["tests_passed"] / self.results["tests_run"] * 100 if self.results["tests_run"] > 0 else 0
        )
...
async def main():
    ...
    parser.add_argument("--url", default="http://localhost:8080", help="Base URL of the service to test")
    parser.add_argument("--wait", type=int, default=0, help="Seconds to wait before starting tests")
