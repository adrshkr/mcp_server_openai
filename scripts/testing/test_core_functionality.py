...
from mcp_server_openai.tools.enhanced_icon_generator import IconRequest
from mcp_server_openai.tools.enhanced_image_generator import ImageRequest
from mcp_server_openai.tools.enhanced_ppt_generator import LLMClient, PPTRequest, PPTResponse
...
async def test_dataclasses():
    ...
    try:
        # Test PPTRequest
        ppt_req = PPTRequest(
            brief="A test presentation", notes=["Note 1", "Note 2"], target_length="3 slides", model_type="gpt-4o"
        )
        print(f"  ✅ PPTRequest: {ppt_req.brief}")

        # Test PPTResponse
        ppt_resp = PPTResponse(status="success", file_path="test.pptx", slides_count=3, file_size=1024)
        print(f"  ✅ PPTResponse: {ppt_resp.status}")

        # Test ImageRequest
        img_req = ImageRequest(query="technology", content_type="presentation", style="professional")
        print(f"  ✅ ImageRequest: {img_req.query}")

        # Test IconRequest
        icon_req = IconRequest(description="business icon", content_type="presentation")
        print(f"  ✅ IconRequest: {icon_req.description}")

        # Test ContentRequest
        content_req = ContentRequest(title="Test Content", brief="A test content", notes=["Note 1", "Note 2"])
...
async def test_content_creation():
    ...
    try:
        ...
        outline = ContentOutline(
            title="Test Content",
            sections=[
                {"title": "Introduction", "type": "title", "content": "Welcome to the presentation"},
                {"title": "Main Content", "type": "content", "content": "This is the main content"},
            ],
            total_sections=2,
        ...
        sections = [
            ContentSection(
                title="Introduction", content="Welcome to the presentation", section_type="title", layout="title_slide"
            ),
            ContentSection(
                title="Main Content", content="This is the main content", section_type="content", layout="content_slide"
            ),
        ]
...
async def main():
    ...
    tests = [test_dataclasses(), test_constants(), test_llm_client(), test_content_creation()]
